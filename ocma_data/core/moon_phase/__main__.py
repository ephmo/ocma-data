"""Subpackage for moon phase calculations."""
...
def valid_date(value: str) -> date:
    ...
    if not (YEAR_START <= parsed_date.year <= YEAR_END):
        raise argparse.ArgumentTypeError(f"Year must be between {YEAR_START} and {YEAR_END}.")
    return parsed_date


def main():
    """Parse arguments and print the moon phase for the given date and calendar."""
    parser = argparse.ArgumentParser(description="Calculate the moon phase date.")
    parser.add_argument(
        "-d",
        "--date",
        type=valid_date,
        required=True,
        help="Date to calculate the moon phase",
    )
    parser.add_argument(
        "-c",
        "--calendar",
    ...
    print(get_moon_phase(args.date, args.calendar))
