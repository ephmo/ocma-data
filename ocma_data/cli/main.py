...
def main() -> None:
    ...
    Path(f"{CLIConstants().BUILD_FOLDER}/{CLIConstants().PASCHALION}").mkdir(parents=True, exist_ok=True)

    for calendar_style in CalendarStyles:
        Path(f"{CLIConstants().BUILD_FOLDER}/{calendar_style.value}").mkdir(parents=True, exist_ok=True)
        paschalion_data = {}
        paschalion_data[JsonKeys.PASCHA_DATE.value] = {}

        for current_year in range(CLIConstants().YEAR_START, CLIConstants().YEAR_END):
            paschalion_data[JsonKeys.PASCHA_DATE.value][current_year] = calculate_pascha(
                current_year, calendar_style.value
            )
            ...
            for current_month in range(1, 13):
                ...
                for current_day in range(1, 32):
                    if is_valid_date(current_year, current_month, current_day):
                        ...
                        calendar_data[str(current_year)][str(current_month)][str(current_day)] = {}
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
                            JsonKeys.DATE_OLD.value
                        ] = date_old_new[JsonKeys.DATE_OLD.value]
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
                            JsonKeys.DATE_NEW.value
                        ] = date_old_new[JsonKeys.DATE_NEW.value]
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
                            JsonKeys.WEEKDAY_INDEX.value
                        ] = get_weekday(current_date, calendar_style.value)
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
                            JsonKeys.MOON_PHASE_INDEX.value
                        ] = get_moon_phase(
                            string_to_date(f"{current_year}-{current_month}-{current_day}"),
                            calendar_style.value,
                        )
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
                            JsonKeys.PASCHA_DISTANCE.value
                        ] = pascha_distance(current_date, calendar_style.value)
                        calendar_data[str(current_year)][str(current_month)][str(current_day)][
